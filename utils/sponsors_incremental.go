...
const sponsorDeleteBatchSize = 500

type sponsorTuple struct {
	EventID          uint32
	EditionID        uint32
	SponsorSourceID  uint32
}

func ProcessIncrementalEventSponsor(mysqlDB *sql.DB, clickhouseConn driver.Conn, config shared.Config) error {
	startTime := time.Now()
	log.Println("=== Starting Incremental Event Sponsor Sync ===")
	...
	if len(toDelete) > 0 {
		...
		if err := deleteEventSponsorRowsByPrimaryKey(nativeConn, toDelete, tableName); err != nil {
	...
	if len(toInsert) > 0 {
		...
		shared.WriteIncrementalLog("")
		shared.WriteIncrementalLog(fmt.Sprintf("5. INSERT: %d records into event_sponsors_ch", len(toInsert)))
		if err := InsertEventSponsorChDataIntoTable(clickhouseConn, toInsert, tableName, config.ClickHouseWorkers); err != nil {
			return fmt.Errorf("insert records: %w", err)
		}
		log.Printf("Inserted %d records into event_sponsors_ch", len(toInsert))
	} else {
		shared.WriteIncrementalLog("")
		shared.WriteIncrementalLog("5. INSERT: 0 (no records to insert)")
	}

	log.Printf("Running OPTIMIZE on event_sponsors_ch...")
	optimizeErr := shared.OptimizeSingleTable(clickhouseConn, "event_sponsors_ch", config, "")
	if optimizeErr != nil {
	...
	shared.WriteIncrementalLog(fmt.Sprintf("[%s] INCREMENTAL EVENT SPONSOR SYNC COMPLETED", endTime.Format("2006-01-02 15:04:05")))
	shared.WriteIncrementalLog("")

	log.Println("=== Incremental Event Sponsor Sync Completed ===")
	return nil
}
...
func deleteEventSponsorRowsByPrimaryKey(conn driver.Conn, tuples []sponsorTuple, tableName string) error {
	...
	for batchStart := 0; batchStart < len(tuples); batchStart += sponsorDeleteBatchSize {
		batchEnd := batchStart + sponsorDeleteBatchSize
		...
		for _, t := range batch {
			tupleStrs = append(tupleStrs, fmt.Sprintf("(%d, %d, %d)", t.EventID, t.EditionID, t.SponsorSourceID))
		}

		deleteQuery := fmt.Sprintf(
			`ALTER TABLE %s DELETE WHERE (event_id, edition_id, sponsorSourceId) IN (%s) SETTINGS mutations_sync = 1`,
			tableName, strings.Join(tupleStrs, ","),
		)
		log.Printf("[Query] %s", deleteQuery)

		log.Printf("[EventSponsor DELETE] Batch %d-%d (%d tuples)", batchStart+1, batchEnd, len(batch))
