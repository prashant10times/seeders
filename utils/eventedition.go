...
func formatEconomicImpact(_ int64, economicImpact string) (interface{}, interface{}, interface{}) {
	var economicImpactJSON map[string]interface{}
	if err := json.Unmarshal([]byte(economicImpact), &economicImpactJSON); err != nil {
...
func ProcessEventEditionOnly(mysqlDB *sql.DB, clickhouseConn driver.Conn, esClient *elasticsearch.Client, config shared.Config) {
	...
	for {
		...
		if len(eventIDs) > 0 {
			...
			if len(editionData) > 0 {
				...
				startTime = time.Now()
				esData = fetchElasticsearchDataForEvents(esClient, config.ElasticsearchIndex, eventIDs)
			...
			if len(editionData) > 0 {
				...
				if len(eventIDsForEditions) > 0 {
					log.Printf("Event edition chunk %d: Fetching category names for %d events", chunkNum, len(eventIDsForEditions))
					startTime = time.Now()
					categoryNamesMap = fetchCategoryNamesForEvents(mysqlDB, eventIDsForEditions)
				...
				for eventID, editions := range eventEditions {
					...
					if eventData != nil {
						for _, edition := range editions {
							...
							// Extract keywords only for current edition
							if currentEditionIDs[eventID] == edition["edition_id"].(int64) {
								...
								keywords := extractKeywords(eventName, eventAbbrName, eventDescription, eventPunchline, categoryNames)
...
func fetchCategoryNamesForEvents(db *sql.DB, eventIDs []int64) map[int64][]string {
	if len(eventIDs) == 0 {
...
func extractKeywords(eventName string, eventAbbrName *string, eventDescription *string, eventPunchline *string, categoryNames []string) []string {
